...
export default function OrdersPage() {
    ...
    const getStatusColor = (status) => {
        const statusColors = { PENDING: "bg-blue-50 text-blue-700", PROCESSING: "bg-blue-100 text-blue-800", SHIPPED: "bg-indigo-100 text-indigo-800", DELIVERED: "bg-green-100 text-green-800", CANCELLED: "bg-red-100 text-red-800", REFUNDED: "bg-purple-100 text-purple-800" };
    ...
    return (
        <>
            ...
            {orders.length > 0 && (
                <div className="bg-gradient-to-r from-emerald-50 to-teal-50 rounded-lg border border-emerald-100 shadow-sm p-5 mb-8">
                    <div className="flex flex-col md:flex-row items-start md:items-center justify-between">
                        <div>
                            <div className="text-lg font-medium text-[#003E29] mb-1">Recent Order: #{orders[0].orderNumber}</div>
                        ...
                        <Button className="mt-4 md:mt-0 bg-[#003E29] hover:bg-[#002216] text-white border border-[#D4AF37]/20" onClick={() => router.push(`/account/orders/${orders[0].id}`)}><DynamicIcon name="Eye" className="mr-2 h-4 w-4" />View Order Details</Button>
                    </div>
                </div>
            )}
            ...
            {loadingOrders ? (
                ...
                <div className="bg-white rounded-lg shadow p-8 text-center font-sans">
                    <DynamicIcon name="ShoppingBag" className="h-16 w-16 mx-auto text-gray-400 mb-4" />
                    <h2 className="text-xl font-semibold mb-2">No Orders Found</h2>
                    <p className="text-gray-600 mb-6">You haven&apos;t placed any orders yet.</p>
                    <Link href="/products"><Button className="bg-[#003E29] hover:bg-[#002216] text-white border border-[#D4AF37]/25">Start Shopping</Button></Link>
                </div>
            ) : (
